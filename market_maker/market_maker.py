    ...
    def converge_orders(self, buy_orders, sell_orders):
        ...
        for order in existing_orders:
            try:
                ...
                # Change code to allow for changing the order price when one side of the order filled, not just if the
                #old order is too far way from the relist_interval, use recent_trades(self, symbol) to get the
                # trades that were just made. Check the direction of the past trades vs the current position, get_deta and
                # get_highest_buy and get_lowest_sell to determine which new price to show. self.exchange.get_delta()

                #skew the prices based on  self.running_qty if positive or not and how close to settings.MAX_POSITION
                # and settings.MIN_POSITION
        ...
        # adds in new orders if the limits allow for. Need to change this so new orders don't get added
